    ...
    public class PrometheusFire : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (Projectile.ai[0] == 0 && Projectile.penetrate == 1)
            {
                ...
                velocity.Normalize();
                Vector2 nextVelocity = velocity.RotatedByRandom(MathHelper.PiOver4) * (4f + Main.rand.Next(0, 4));
                Projectile.velocity = nextVelocity;

                for (int i = 0; i < 4; i++)
                {
                    nextVelocity = velocity.RotatedByRandom(MathHelper.PiOver4) * (4f + Main.rand.Next(0, 4));
