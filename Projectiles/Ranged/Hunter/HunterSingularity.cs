...
{
    public class HunterSingularity : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                int i = FindOldestSingularity(player);
        ...
        public void Explode()
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                Dust dust = Dust.NewDustDirect(Projectile.position, Projectile.height, Projectile.width, DustID.ShadowbeamStaff);
                dust.velocity *= 6f;
            }
        }

        readonly object[] shootStats = new object[5];
        int shootTimer = 0;
        ...
        public static int FindOldestSingularity(Player player)
        {
            int timeleft = 3600;
            int result = -1;
            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                ...
                if (bomb.active && bomb.type == ModContent.ProjectileType<HunterSingularity>() && bomb.owner == player.whoAmI && bomb.timeLeft < timeleft)
                {
                    result = i;
