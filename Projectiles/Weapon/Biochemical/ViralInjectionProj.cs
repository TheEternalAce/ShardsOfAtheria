...
namespace ShardsOfAtheria.Projectiles.Weapon.Biochemical
{
    public class ViralInjectionProj : ModProjectile
    {
        public override string Texture => "ShardsOfAtheria/Items/Weapons/Biochemical/ViralInjection";
		...
		{
			DisplayName.SetDefault("Viral Injection");
		}
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			severityChooser.Add(ModContent.BuffType<BasicViralInfectionI>());
			severityChooser.Add(ModContent.BuffType<BasicViralInfectionII>());
			severityChooser.Add(ModContent.BuffType<BasicViralInfectionIII>());
			int severityChoice = severityChooser;
			int virus = Main.LocalPlayer.FindItem(ModContent.ItemType<Virus>());

			if (!target.HasBuff(ModContent.BuffType<BasicViralInfectionI>()) && !target.HasBuff(ModContent.BuffType<BasicViralInfectionII>())
                && !target.HasBuff(ModContent.BuffType<BasicViralInfectionIII>()) && Main.player[Projectile.owner].inventory[virus].stack >= 15)
			{
				target.AddBuff(severityChoice, 10 * 60);
				Main.player[Projectile.owner].inventory[virus].stack -= 15;
			}
		}
    }
}
