...
{
    public class BacterialInjectionProj : ModProjectile
    {
        public override string Texture => "ShardsOfAtheria/Items/Weapons/Biochemical/BacterialInjection";
		...
		{
			DisplayName.SetDefault("Bacterial Injection");
		}
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			int bacteria = Main.LocalPlayer.FindItem(ModContent.ItemType<Bacteria>());
			...
			if (!target.HasBuff(ModContent.BuffType<BasicBacterialInfectionI>()) && !target.HasBuff(ModContent.BuffType<BasicBacterialInfectionII>())
                && !target.HasBuff(ModContent.BuffType<BasicBacterialInfectionIII>()) && Main.player[Projectile.owner].inventory[bacteria].stack >= 15)
			{
				target.AddBuff(severityChoice, 10 * 60);
				Main.LocalPlayer.inventory[bacteria].stack -= 15;
			}
		}
    }
}
