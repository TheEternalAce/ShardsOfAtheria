        ...
        public override void AI()
        {
            ...
            if (Main.myPlayer == Projectile.owner)
            {
                ...
                if (Main.mouseLeft && !Main.LocalPlayer.mouseInterface && player.HeldItem.type != ModContent.ItemType<SoulExtractingDagger>())
                {
                    shootTimer++;
                    if (shootTimer == 1)
                        Projectile.NewProjectile(Projectile.GetProjectileSource_FromThis(), Projectile.Center, Vector2.Normalize(Main.MouseWorld - Projectile.Center) * 10, ProjectileID.MiniRetinaLaser, 50, 2f, player.whoAmI);
                    if (shootTimer >= 60)
                        shootTimer = 0;
                }
