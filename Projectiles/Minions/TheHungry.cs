...
namespace ShardsOfAtheria.Projectiles.Minions
{
    public class TheHungry : ModProjectile
    {
        ...
        {
            if (owner.dead || !owner.active || owner.GetModPlayer<SlayerPlayer>().WallSoul != SoulCrystalStatus.Absorbed)
                return false;
            return true;
		}
        ...
        public override void Kill(int timeLeft)
        {
			...
			Gore.NewGore(Projectile.Center, new Vector2(Main.rand.Next(-6, 7), Main.rand.Next(-6, 7)), 132);
			Gore.NewGore(Projectile.Center, new Vector2(Main.rand.Next(-6, 7), Main.rand.Next(-6, 7)), 133);
        }
    }
}
