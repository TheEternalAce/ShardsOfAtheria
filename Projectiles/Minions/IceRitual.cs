...
{
    public class IceRitual : ModProjectile
	{
        ...
        {
            DisplayName.SetDefault("Ice Ritual");
        }
					...
					if (Projectile.ai[1] == 1)
					{
						...
						if (owner.GetModPlayer<SlayerPlayer>().lunaticCircleFragments > 1)
						{
							...
							for (int i = 0; i < numberProjectiles; i++)
							{
								...
								Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, perturbedSpeed * 16, ModContent.ProjectileType<IceFragment>(), 90, 1, owner.whoAmI);
							}
						}
		...
		{
			if (owner.dead || !owner.active || owner.GetModPlayer<SlayerPlayer>().LunaticSoul == SoulCrystalStatus.None)
				return false;
			return true;
		}
