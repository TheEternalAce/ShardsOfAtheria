...
namespace ShardsOfAtheria.NPCs
{
	public class HarpyIce : ModNPC
    {
        ...
        public override float SpawnChance(NPCSpawnInfo spawnInfo)
        {
            ...
            if (!(spawnInfo.player.ZoneHallow || spawnInfo.player.ZoneCrimson || spawnInfo.player.ZoneCorrupt || Main.pumpkinMoon
                || Main.snowMoon || spawnInfo.player.ZoneTowerNebula || spawnInfo.player.ZoneTowerVortex || spawnInfo.player.ZoneTowerSolar
                || spawnInfo.player.ZoneTowerStardust || spawnInfo.playerSafe) && spawnInfo.player.ZoneSnow && spawnInfo.player.ZoneOverworldHeight)
                return .25f;
            return 0f;
        }
        ...
        public override void ModifyNPCLoot(NPCLoot npcLoot)
        {
            ...
            if (Main.rand.NextFloat() < .5f)
                Item.NewItem(NPC.getRect(), ItemID.IceBlock, Main.rand.Next(3, 6));
            if (Main.rand.NextFloat() < .5f)
                Item.NewItem(NPC.getRect(), ItemID.SnowBlock, Main.rand.Next(3, 6));
        }
